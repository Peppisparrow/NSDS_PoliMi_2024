...
public class EventEnrichment {
    public static void main(String[] args) throws TimeoutException, StreamingQueryException {
        ...
        // TODO
        Dataset<Row> inAlias = inStream.as("in");
        Dataset<Row> prod = productsClassification.as("prod");
        final Dataset<Row> enrichedStream = inStream
                .join(productsClassification, col("product").equalTo(col("product")));

        // Define a windowed aggregation to count products by class
        Dataset<Row> productCounts = enrichedStream
                .groupBy(
                        window(col("timestamp"), "30 seconds", "5 seconds"),
                        col("classification")
                )
                .count()
                .withColumnRenamed("count", "class_count");

        // Start the query to write the results to console
        StreamingQuery query = productCounts
                .writeStream()
                .outputMode("update")
                .format("console")
